...
```{r data}
health_facilities <- read.csv("Data/kaduna_health_sub.csv")
...
```{r view}
datatable(head(health_facilities[8:18])) # printing first rows of dataset
```
...
- Idea: optimize the  partitioning of the area into polygons such that each polygon contains exactly one point.
...
```{r, pop distribution}
hist(pop_per_voronoi$population, breaks= 50,
     main= "",
     xlab= "Population",
     col= "grey")
